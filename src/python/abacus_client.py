...
# Importar cliente otimizado
try:
    from ..integrations.abacus.client import AbacusOptimizedClient
...
def create_abacus_client(use_optimized: bool = True, **kwargs) -> AbacusClient:
    """
    Factory function para criar cliente Abacus
    
    Args:
        use_optimized: Se deve usar cliente otimizado quando disponível
        **kwargs: Argumentos para o cliente
        
    Returns:
        Instância do cliente (otimizado se disponível)
    """
    if use_optimized and OPTIMIZED_CLIENT_AVAILABLE:
        logger.info("Usando cliente Abacus otimizado")
        # Converter argumentos para o cliente otimizado
        optimized_kwargs = {}
        if 'api_key' in kwargs:
            optimized_kwargs['api_key'] = kwargs['api_key']
        if 'max_retries' in kwargs:
            optimized_kwargs['max_retries'] = kwargs['max_retries']
        
        return AbacusOptimizedClient(**optimized_kwargs)
    else:
        logger.info("Usando cliente Abacus padrão")
        return AbacusClient(**kwargs)


# Exemplo de uso
if __name__ == "__main__":
    # Configurar cliente (tentará usar otimizado primeiro)
    client = create_abacus_client()
